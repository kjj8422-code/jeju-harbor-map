...
/* 리포트가 저절로 닫히고 날이 밝기까지 (초).
   예전에는 「다음 날로」를 눌러야만 넘어갔습니다 — 밤마다 흐름이 끊겼습니다. */
export const REPORT_AUTO_SEC = 8;
...
/* ★ 타격감 강화 (팀장님: "적들 몰려올 때 싸우는 게 좀 더 박진감 있게").
   때린 맛은 세 가지가 함께 움직여야 납니다 —
     밀려나는 거리(넉백) · 멈추는 시간(히트스톱) · 화면이 흔들리는 폭.
   하나만 키우면 어색해집니다. 셋을 같이 올렸습니다. */
export const KNOCKBACK = 125;           // 90 → 125
export const KNOCKBACK_SKILL = 300;     // 210 → 300
export const HITSTOP = 0.075;           // 0.055 → 0.075
export const HITSTOP_CRIT = 0.14;       // 치명타는 두 배 가까이 — "묵직하게 박혔다"
export const HITSTOP_ULT = 0.2;         // 궁극기
...
/* body 는 3D 로 어떻게 생겼는지입니다 — 'man' 사람, 'beast' 네발짐승, 'undead' 시체.
   voice 는 다가올 때 우는 소리입니다 (js/audio.js 의 이름과 맞춰야 합니다).
   tip 은 리포트·안내에 그대로 나가는 "어떻게 상대하나" 한 줄입니다. */
export const MONSTER_KINDS = {
  normal: { name:'졸개',     body:'man',    voice:null,     hpMul:1.0, spdMul:1.0,  dmgMul:1.0, armor:0,    scale:1.00, color:0xE0554A,
            tip:'기본 몬스터입니다. 함정과 병사로 충분히 정리됩니다.' },
  fast:   { name:'기병',     body:'man',    voice:null,     hpMul:0.7, spdMul:1.5,  dmgMul:0.9, armor:0,    scale:0.92, color:0xE08B3C,
            tip:'빨라서 함정 한 칸은 그냥 지나칩니다. 함정을 두세 칸 이어 까세요.' },
  tank:   { name:'방패병',   body:'man',    voice:null,     hpMul:2.3, spdMul:0.68, dmgMul:1.2, armor:0.30, scale:1.28, color:0x7D8A96,
            tip:'목책이 오래 버텨야 합니다. 성을 올리고 방패 용병을 세우세요.' },
  elite:  { name:'정예',     body:'man',    voice:null,     hpMul:3.2, spdMul:0.95, dmgMul:1.6, armor:0.15, scale:1.38, color:0x9B6FC9,
            tip:'붉은 원이 차오르면 걸어서 물러나 흘리고, 스킬로 끊으세요.' },

  /* ── 짐승·역병 (팀장님 요청: 늑대·좀비·호랑이) ──────────────────
     사람 병사만 나오면 아홉 번의 밤이 다 같아 보입니다.
     생김새(네발/시체)와 우는 소리가 다르면 "무엇이 오는지" 가 소리만으로 구분됩니다. */
  wolf:   { name:'들개떼',   body:'beast',  voice:'howl',   hpMul:0.5, spdMul:1.8,  dmgMul:0.7, armor:0,    scale:0.78, color:0x6E6355,
            pack:3,
            tip:'셋씩 붙어 다닙니다. 하나하나는 약하니 광역 스킬로 한 번에 쓸어내세요.' },
  /* spdMul 을 0.45 → 0.62 로 올렸습니다.
     0.45 면 지도를 건너오는 데 너무 오래 걸려서, 플레이어가 **방어선이 아니라 들판에서**
     마중 나가 잡게 됩니다 (자동 플레이의 44일 함정 처치 비율이 0% 였습니다).
     느린 맛은 살리되, 제 발로 함정까지는 오게 만드는 값입니다. */
  zombie: { name:'역병 시체', body:'undead', voice:'groan',  hpMul:3.1, spdMul:0.62, dmgMul:1.1, armor:0,    scale:1.10, color:0x6F8A5C,
            tip:'느린 대신 아주 질깁니다. 함정 위에 오래 세워두면 알아서 녹습니다.' },
  tiger:  { name:'맹호',     body:'beast',  voice:'roar',   hpMul:2.2, spdMul:1.35, dmgMul:2.0, armor:0.10, scale:1.30, color:0xD9873A,
            tip:'빠르고 아픕니다. 정면으로 받지 말고 목책 뒤 함정으로 유인하세요.' }
};
...
export const WAVES = [
  ...
  { day:22, act:1, name:'산적과 들개', note:'다수 약체 · 들개가 처음 섞입니다',
    count:18, hp:46, spd:78, dmg:8, sides:2,
    mix:[['normal',0.7],['wolf',0.3]],
  ...
  { day:33, act:1, name:'산적 두목', note:'1막 보스 · 함정의 가치를 배우는 구간',
    ...
    mix:[['normal',0.6],['fast',0.2],['wolf',0.2]],
  ...
  { day:44, act:2, name:'오환 기병대', note:'2막 시작 · 빠른 적과 역병 시체가 함께 옵니다',
    count:22, hp:62, spd:92, dmg:12, sides:2,
    mix:[['normal',0.3],['fast',0.45],['wolf',0.1],['zombie',0.15]],
    advice:'기병은 함정을 금방 빠져나갑니다. 함정 한 칸보다 두세 칸을 잇는 쪽이 낫습니다.<br>'
         + '<b>역병 시체</b>는 정반대입니다 — 느리고 질기니 함정 위에 오래 세워두면 알아서 녹습니다.' },
  { day:55, act:2, name:'남만 상군', note:'단단한 방패병 · 목책이 오래 버텨야 합니다',
    count:24, hp:78, spd:74, dmg:14, sides:3,
    mix:[['normal',0.3],['tank',0.4],['zombie',0.3]],
    advice:'방패병은 목책을 오래 두드립니다. 성을 석성 이상으로 올리고 병사를 길목에 세우세요.' },
  { day:66, act:2, name:'맹획의 맹수 부대', note:'2막 보스 · 세 방향 · 맹호가 처음 나옵니다',
    count:26, hp:88, spd:82, dmg:15, sides:3,
    mix:[['normal',0.25],['fast',0.2],['tank',0.25],['wolf',0.15],['tiger',0.15]],
  ...
  { day:77, act:3, name:'위군 선봉', note:'3막 시작 · 정예가 섞여 들어옵니다',
    ...
    mix:[['normal',0.25],['fast',0.2],['tank',0.2],['elite',0.1],['zombie',0.15],['tiger',0.1]],
    advice:'정예 한 마리가 졸개 셋보다 아픕니다. 붉은 원이 차오르면 걸어서 물러나 흘리고 스킬로 끊으세요.' },
  { day:88, act:3, name:'조조의 정예', note:'사방에서 몰려옵니다',
    count:32, hp:120, spd:90, dmg:18, sides:4,
    mix:[['normal',0.24],['fast',0.18],['tank',0.16],['elite',0.15],['wolf',0.09],['zombie',0.09],['tiger',0.09]],
  ...
  { day:99, act:3, name:'최후의 대란', note:'최종 보스 · 99일의 끝',
    ...
    mix:[['normal',0.2],['fast',0.17],['tank',0.15],['elite',0.18],['wolf',0.1],['zombie',0.1],['tiger',0.1]],
...
/* ★ 채집 속도를 낮췄습니다 (팀장님: "자원을 캐는 속도가 너무 빠르던데").
   예전 값은 목재 3.4/초 — 8초짜리 하루에 나무 한 그루 옆에 서 있기만 해도 27이 쌓였습니다.
...
/* ==================================================================
   난이도 (초급 · 중급 · 상급)
   ------------------------------------------------------------------
   팀장님: "자원이 많아서 설치를 많이 하니 난이도가 너무 쉬운 것 같다.
            모드를 초급·중급·상급으로 나누는 게 어떨까?"

   난이도를 "적 체력만 올리기" 로 만들면 같은 게임을 더 오래 하는 것뿐입니다.
   그래서 **세 축을 함께** 움직입니다.
     ① 자원이 얼마나 빨리 모이나 (gather · nodeMul)  → 고민의 밀도
     ② 적이 얼마나 많고 단단한가 (waveMul · hpMul)   → 방어선의 부담
     ③ 실수를 얼마나 봐주나 (baseHpMul · respawnMul) → 한 번의 실수 비용
   초급은 ①을 넉넉히 주고 ③으로 감싸고, 상급은 ①을 조이고 ②를 올립니다.
   ================================================================== */
export const DIFFS = [
  { id:'easy', name:'초급', icon:'🌱', color:'#5FAE72',
    tag:'처음이라면 여기부터',
    gather:1.50, nodeMul:1.25, waveMul:0.80, hpMul:0.80, baseHpMul:1.30, respawnMul:0.7, shardMul:1.0,
    desc:'자원이 빨리 모이고 적이 약합니다. 조작과 흐름을 익히는 판입니다.',
    detail:'채집 +50% · 자원지 +25% · 적 수 -20% · 적 체력 -20% · 거점 체력 +30% · 부활 30% 빠름' },
  { id:'normal', name:'중급', icon:'⚔️', color:'#E0B44A',
    tag:'기본 균형',
    gather:1.00, nodeMul:1.00, waveMul:1.00, hpMul:1.00, baseHpMul:1.00, respawnMul:1.0, shardMul:1.2,
    desc:'자원이 빠듯합니다. 어디에 목책을 세우고 어디에 함정을 깔지 고민해야 합니다.',
    detail:'기준값 · 옥새 조각 +20%' },
  { id:'hard', name:'상급', icon:'🔥', color:'#C6412F',
    tag:'한 번의 실수가 판을 가릅니다',
    gather:0.72, nodeMul:0.85, waveMul:1.25, hpMul:1.30, baseHpMul:0.85, respawnMul:1.4, shardMul:1.5,
    desc:'자원이 모자랍니다. 함정 하나의 위치가 승패를 가릅니다. 정수를 아껴 쓰세요.',
    detail:'채집 -28% · 자원지 -15% · 적 수 +25% · 적 체력 +30% · 거점 체력 -15% · 부활 40% 느림 · 옥새 +50%' }
];
export const DEFAULT_DIFF = 'normal';
export const diffOf = id => DIFFS.find(d => d.id === id) || DIFFS[1];

/* ==================================================================
   서지(進擊) — "적 본대가 밀려온다"
   ------------------------------------------------------------------
   예전에는 밤이 시작될 때 그 밤의 적이 **전부 한꺼번에** 생겼습니다.
   그래서 "조용히 하나둘 나타나는" 것도 아니고 "몰려오는" 것도 아닌,
   그냥 처음부터 지도에 흩어져 있는 상태였습니다.

   이제 밤을 **여러 번의 진격**으로 나눕니다.
     · 밤이 시작되면 선발대(firstPct)만 옵니다.
     · 나머지는 몇 번에 나눠 **한 덩어리씩** 밀고 들어옵니다.
     · 진격이 시작될 때마다 경고 배너 + 화면 흔들림 + 붉은 섬광 + 징·함성.
   같은 수의 적이라도 "지금 밀려온다" 는 순간이 생기면 밤이 사건이 됩니다.
   ================================================================== */
export const SURGE = {
  firstPct: 0.42,          // 밤이 열릴 때 오는 선발대 비율
  minGroups: 2,            // 나머지를 최소 몇 번에 나눠 보낼지
  maxGroups: 5,
  /* 간격을 7.0 → 5.0 으로 좁혔습니다. 너무 띄우면 무리가 하나씩 도착해서
     장수가 차례로 처리해버립니다 — "몰려온다" 가 아니라 "줄 서서 온다" 가 됩니다. */
  gapSec: 5.0,             // 진격 사이 간격(초) — 뒤로 갈수록 짧아집니다
  gapMin: 2.4,
  bannerSec: 1.9,          // 배너가 떠 있는 시간
  shake: 0.9,              // 화면 흔들림 세기
  /* 웨이브가 뒤로 갈수록 진격 횟수가 늘고 간격이 짧아집니다 */
  groupsFor: i => Math.min(5, 2 + Math.floor(i / 2)),
  gapFor:    i => Math.max(2.4, 5.0 - i * 0.3),
  names: ['적 본대가 밀려온다!!', '후군이 붙었다!!', '측면에서 밀고 들어온다!!',
          '전군 돌격이다!!', '마지막 물결이다!!']
};

/* ==================================================================
   진형 전진 — 무리가 대열을 유지하며 밀고 들어옵니다
   ------------------------------------------------------------------
   개별 AI 로만 두면 빠른 적이 먼저 도착하고 느린 적이 줄줄이 따라옵니다.
   그러면 "부대" 가 아니라 "행렬" 로 보입니다.
   한 진격으로 온 무리는 **가장 느린 놈에 속도를 맞춰** 뭉쳐서 전진하다가,
   방어선(breakDist)에 닿거나 피해를 입으면 대열이 풀리고 각자 싸웁니다.
   ================================================================== */
export const FORMATION = {
  spread: 22,              // 대열 좌우 간격(월드 단위)
  rows: 4,                 // 한 줄에 몇 마리
  breakDist: 5.5 * 28,     // 거점에서 이만큼 가까워지면 대열이 풀립니다 (5.5칸)
  keep: 0.94,              // 대열 유지 중 속도 배율 (가장 느린 놈 기준)
  voiceGap: 1.1            // 같은 울음소리가 겹치지 않게 두는 최소 간격(초)
};
